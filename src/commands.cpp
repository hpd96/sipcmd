...
bool Command::Run(std::vector<Command *> &sequence, const std::string &loopsuffix) {
...
////
// Call
bool Call::ParseCommand(const char **cmds, std::vector<Command *> &sequence) {
    size_t i = 0U;
    for (; (*cmds)[i] && (*cmds)[i] != ';'; i++) {
    }
...
bool Call::RunCommand(const std::string &loopsuffix) {
    ...
    if (rp.Find('@') == P_MAX_INDEX && !gw.IsEmpty()) {
        cout << "TestPhone::Main: calling \"" << rp << "\" using gateway \"" << gw << "\""
...
////
// Answer
bool Answer::ParseCommand(const char **cmds, std::vector<Command *> &sequence) {
    size_t i = 0U;
    for (; (*cmds)[i] && (*cmds)[i] != ';'; i++) {
    }
...
bool Answer::RunCommand(const std::string &loopsuffix) {
    ...
    if (!tpstate.GetManager()->IsListenerUp() && !tpstate.GetManager()->StartListener()) {
        return false;
    }
...
////
// Hangup
bool Hangup::ParseCommand(const char **cmds, std::vector<Command *> &sequence) {
    for (; **cmds && **cmds != ';'; (*cmds)++) {
    }
...
////
// DTMF
bool DTMF::ParseCommand(const char **cmds, std::vector<Command *> &sequence) {
    size_t i = 0U;
    for (; (*cmds)[i] && (*cmds)[i] != ';' && strchr("0123456789#*ABCD!", (*cmds)[i]); i++) {
    }
...
////
// Voice
bool Voice::ParseCommand(const char **cmds, std::vector<Command *> &sequence) {
    size_t i = 0U;
    for (; (*cmds)[i] && (*cmds)[i] != ';'; i++) {
    }
    if (!i) {
...
bool Voice::RunCommand(const std::string &loopsuffix) {
    ...
    // playback audio
    bool ok = TPState::Instance().GetPlayBackAudio().PlaybackAudioFile(audiofilename);
...
////
// Record
bool Record::ParseCommand(const char **cmds, std::vector<Command *> &sequence) {
...
bool Record::RunCommand(const std::string &loopsuffix) {
    ...
    if (iterationsuffix) {
        ...
        if (ext == P_MAX_INDEX) {
            ...
            filename = audiofilename.Left(ext) + loopsuffix + audiofilename.Mid(ext);
        }
    } else {
    ...
    // record audio
    bool ok = TPState::Instance().GetRecordAudio().RecordAudioFile(filename, append, silence, millis);
...
////
// Wait
bool Wait::ParseCommand(const char **cmds, std::vector<Command *> &sequence) {
...
bool Wait::RunCommand(const std::string &loopsuffix) {
    ...
    for (int n = millis / WAIT_SLEEP_ACCURACY; n >= 0; n--) {
        // silence detection
        if (silence && TPState::Instance().IsSilent(WAIT_SILENCE_TIME_IN_MS * BYTES_PER_MILLIS)) {
            cout << "Wait: silence detected" << endl;
            break;
        }
        // activity detection
        else if (activity && TPState::Instance().IsActive(WAIT_ACTIVITY_TIME_IN_MS * BYTES_PER_MILLIS)) {
            cout << "Wait: activity detected" << endl;
            break;
        }
        // disconnect detection
        if (closed && (TPState::Instance().GetState() == TPState::TERMINATED ||
...
////
// Label
bool Label::ParseCommand(const char **cmds, std::vector<Command *> &sequence) {
    size_t i = 0U;
    for (; (*cmds)[i] && (*cmds)[i] != ';'; i++) {
    }
    if (!i) {
...
////
// Loop
bool Loop::PopToInnerSequence(PString &label, std::vector<Command *> &sequence) {
    while (!sequence.empty()) {
...
bool Loop::ParseCommand(const char **cmds, std::vector<Command *> &sequence) {
...
bool Loop::RunCommand(const std::string &loopsuffix) {
    ...
    do {
        ...
        cout << "Loop: iteration \"" << newsuffix.str() << "\" at " << ctime_r(&secsnow, buf) << endl;
