...
class LocalEndPoint : public OpalLocalEndPoint {
   ...
   public:
    ...
    virtual PSafePtr<OpalConnection> MakeConnection(OpalCall &call, const PString &party, void *userData = NULL,
                                                    unsigned int options = 0,
                                                    OpalConnection::StringOptions *opts = NULL);

    virtual OpalLocalConnection *CreateConnection(OpalCall &call, void *userData, unsigned options,
                                                  OpalConnection::StringOptions *strOptions);

    // get media data for transmission
    virtual bool OnReadMediaData(const OpalLocalConnection &connection, const OpalMediaStream &mediaStream, void *data,
                                 PINDEX size, PINDEX &length);

    // receive media data
    virtual bool OnWriteMediaData(const OpalLocalConnection &connection, const OpalMediaStream &mediaStream,
...
class RTPSession : public RTP_UDP {
   ...
   public:
    enum Payload { PCM16, G711_ALAW, G711_ULAW };
...
class Manager : public OpalManager {
   ...
   public:
    ...
    void SetListenMode(const bool m) { listenmode = m; }
    ...
    virtual bool OnIncomingConnection(OpalConnection &connection, unsigned opts,
