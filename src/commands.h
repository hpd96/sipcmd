...
class Command {
   ...
   public:
    ...
    static bool Run(std::vector<Command *> &sequence, const std::string &loopsuffix = "");

    // deletes all commands in the sequence
    static void DeleteSequence(std::vector<Command *> &sequence);

    // returns the error message
    static const std::string &GetErrorString(void) { return errorstring; }

    // parses this command
    // -appends command to the vector
    // -increments cmds pointer
    // -returns whether successful or not
    virtual bool ParseCommand(const char **cmds, std::vector<Command *> &sequence) = 0;
...
class Loop : public Command {
   private:
    ...
    bool PopToInnerSequence(PString &label, std::vector<Command *> &sequence);

   public:
    ~Loop() { Command::DeleteSequence(loopedsequence); }
