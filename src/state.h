...
class TPState {
   ...
   public:
    enum TPConnState { STARTING = 0, CONNECTING = 1, ESTABLISHED = 2, CLOSED = 3, TERMINATED = 666 };

    enum TPProtocol { SIP, H323, RTP };

    static TPState &Instance() {
        if (!instance) instance = new TPState();
        return *instance;
    }

    void SetState(TPConnState newstate) {
        stateSync.Wait();
        if (state != TERMINATED) state = newstate;
        if (someonewaiting)
    ...
    TestChanAudio &GetRecordAudio() { return recordaudio; }
